...
Reads edge-edge narrowphase debug text from stdin or a file, parses vertices at t0/t1 and the
EE line (TOI interval and edge parameters), interpolates a TOI pose (mid/min/max), and produces
a single interactive 3D overlay (t0 vs TOI). If 'reflected' vertices are also present,
they are drawn as a third pose.

Usage:
  cat debug.log | edge_edge_cli_interactive.py --prefix out/edge_edge --toi mid
  edge_edge_cli_interactive.py debug.log --prefix edge_edge --toi max --open
...
def parse_log(text: str) -> Dict[str, np.ndarray]:
    ...
    if toi_match is not None:
        data["toi"] = np.array(
            [float(toi_match.group("toi0")), float(toi_match.group("toi1"))],
            dtype=float,
        )
        data["a_para"] = np.array(
            [float(toi_match.group("a0")), float(toi_match.group("a1"))], dtype=float
        )
        data["b_para"] = np.array(
            [float(toi_match.group("b0")), float(toi_match.group("b1"))], dtype=float
        )
...
def overlay_figure(
    ...
    def point_trace(p, name, symbol="circle", size=5):
    ...
    # t0
    traces += [
        line_trace(A_v0_t0, A_v1_t0, "A t0"),
        line_trace(B_v0_t0, B_v1_t0, "B t0"),
    ]
    pa0 = pseg(A_v0_t0, A_v1_t0, ta)
    pb0 = pseg(B_v0_t0, B_v1_t0, tb)
    traces += [
        point_trace(pa0, "A@t(mid) t0", symbol="circle"),
        point_trace(pb0, "B@t(mid) t0", symbol="square"),
    ]
    traces += [line_trace(pa0, pb0, f"sep t0 ≈ {sep_t0:.6e}", dash="dot", width=2)]

    # TOI
    traces += [
        line_trace(A_v0_toi, A_v1_toi, f"A TOI({toi_mode})", dash="dash"),
        line_trace(B_v0_toi, B_v1_toi, f"B TOI({toi_mode})", dash="dash"),
    ]
    pa1 = pseg(A_v0_toi, A_v1_toi, ta)
    pb1 = pseg(B_v0_toi, B_v1_toi, tb)
    traces += [
        point_trace(pa1, "A@t(mid) TOI", symbol="circle-open"),
        point_trace(pb1, "B@t(mid) TOI", symbol="square-open"),
    ]
    traces += [line_trace(pa1, pb1, f"sep TOI ≈ {sep_toi:.6e}", dash="dot", width=2)]
    ...
    if have_ref:
        traces += [
            line_trace(A_v0_ref, A_v1_ref, "A reflected", dash="longdash"),
            line_trace(B_v0_ref, B_v1_ref, "B reflected", dash="longdash"),
        ]
        ...
        traces += [
            point_trace(pa2, "A@t(mid) reflected", symbol="diamond", size=6),
            point_trace(pb2, "B@t(mid) reflected", symbol="x", size=6),
        ]
        traces += [
            line_trace(pa2, pb2, f"sep reflected ≈ {sep_ref:.6e}", dash="dot", width=2)
        ]
...
def main():
    ap = argparse.ArgumentParser(
        description="Interactive overlay (t0 vs TOI, optional reflected) for narrowphase edge-edge logs.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=textwrap.dedent(
            """
          Examples:
            cat debug.log | edge_edge_cli_interactive.py --prefix out/edge_edge --toi mid --open
            edge_edge_cli_interactive.py debug.log --prefix edge_edge --toi max
        """
        ),
    )
    ap.add_argument(
    ...
    ap.add_argument(
        "--prefix", default="edge_edge", help="Output file prefix (default: edge_edge)"
    )
    ap.add_argument(
        "--toi",
        choices=["mid", "min", "max"],
