...
ModelLoaderWidget::ModelLoaderWidget(UIContext& ui_context, EngineContext& engine_context)
...
bool ModelLoaderWidget::load_model_from_path(const std::string& path) {
    ...
    ui_ctx_.p_surface = py::registerSurfaceMesh("model", engine_ctx_.V, engine_ctx_.F);
    ...
    if (ImGui::CollapsingHeader("Model Loader", ImGuiTreeNodeFlags_DefaultOpen)) {
        if (ImGui::Button("Load Model")) {
            ...
            std::vector<std::string> filters = {
                "All Supported Formats", "*.obj *.off *.ply *.stl",
                "Wavefront OBJ", "*.obj",
                "OFF Files", "*.off",
                "Stanford PLY", "*.ply", 
                "STL Files", "*.stl",
