...
EventFlag MeshStatisticWidget::draw() {
  ...
  if (ImGui::CollapsingHeader("Mesh Statistics",
    ...
    ImGui::Text("Vertex Num: %d", vnum);
    ImGui::Text("Face Num: %d", fnum);
    ...
    ImGui::Text("Dimensions: %.3f x %.3f x %.3f", bbox_max.x - bbox_min.x,
                bbox_max.y - bbox_min.y, bbox_max.z - bbox_min.z);
