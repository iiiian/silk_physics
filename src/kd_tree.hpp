...
struct KDNode {
  ...
  Eigen::RowVector3d node_vert = {0, 0, 0};
  size_t depth = 0;
  // for leaf node only
...
class KDTree {
  ...
  const Eigen::MatrixX3d *_p_verts = nullptr;
  ...
  void update_closest(const Eigen::RowVector3d &point, const KDNode *p_node,
                      double &min_dist_square, Eigen::Index &target_idx) const;

  void update_neighbors(const Eigen::RowVector3d &point, double radius,
                        const KDNode *p_node,
 ...
 public:
  ...
  const double _unbalance_node_threshold = 0.3;
  ...
  void init(const Eigen::MatrixX3d *p_verts);
  void clear();
  std::vector<Eigen::Index> find_neighbors(const Eigen::RowVector3d &point,
                                           double radius) const;
  Eigen::Index find_closest(const Eigen::RowVector3d &point) const;
