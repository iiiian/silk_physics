...
ConfigWidget::ConfigWidget(Context& ctx,
                           std::function<void(jsonPath)> status)
...
void ConfigWidget::draw() {
  ...
  if (ImGui::CollapsingHeader((title_ + "##config").c_str(),
    ...
    if (ImGui::Checkbox("Use Default Json File##config_checkbox", &useDefault)) {
    ...
    if (ImGui::Button("Load JSON##config_btn")) {
      ...
      std::vector<std::string> results =
          pfd::open_file("Select JSON config", ".", filters, pfd::opt::none).result();

      if (!results.empty()) {
        const std::string& picked = results[0];

        if (!is_json_path(picked)) {
          UI_LOGE("[Config] Invalid file type. Please select a .json file: {}", picked);
          ...
          UI_LOGI("[Config] Selected JSON file name: {}", sel.filename().string());
