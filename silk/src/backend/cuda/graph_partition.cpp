...
void graph_partition(ctd::span<int> row_ptr, ctd::span<int> cols,
  ...
  static_assert(sizeof(int) == sizeof(kaminpar_node_id_t),
                "int and KaMinPar node IDs differ in size");
  static_assert(sizeof(int) == sizeof(kaminpar_edge_id_t),
                "int and KaMinPar edge IDs differ in size");
  static_assert(sizeof(int64_t) == sizeof(kaminpar_edge_weight_t),
                "int64 and KaMinPar edge weights differ in size (enable "
  ...
  kaminpar_borrow_and_mutate_graph(partitioner, node_num, xadj, adjncy, nullptr,
                                   adjwgt);
  ...
  kaminpar_compute_partition_with_max_block_weights(
      partitioner, part_num, max_block_weights.data(), partition.data());
