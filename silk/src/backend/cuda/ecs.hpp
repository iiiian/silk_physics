...
#define ECS_X_MACRO(X)                                      \
  X(::silk::ClothConfig, cloth_config)                      \
  X(::silk::CollisionConfig, collision_config)              \
  X(::silk::TriMesh, tri_mesh)                              \
  X(::silk::Pin, pin)                                       \
  X(::silk::ClothTopology, cloth_topology)                  \
  X(::silk::cuda::ClothSolverContext, cloth_solver_context) \
  X(::silk::cuda::ObjectState, object_state)                \
  X(::silk::cuda::ObstaclePosition, obstacle_position)      \
  X(::silk::cuda::ObjectCollider, object_collider)
