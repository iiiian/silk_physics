...
#pragma unroll
  for (int i = 0; i < 3; ++i) {
    atomicAdd(target_sum.data() + offset + i, reflection(i));
...
__global__ void compute_all_barriers(ctd::span<const Collision> collisions,
  ...
  if (filter == BarrierCollisionFilter::InitialContactsOnly &&
      !c.is_initial_contact) {
    return;
  }
  if (filter == BarrierCollisionFilter::LineSearchOnly &&
      c.is_initial_contact) {
    return;
  }
...
EqualityConstraints gather_barrier_constraints(
  ...
  compute_all_barriers<<<grid_num, 128, 0, rt.stream.get()>>>(
      collisions, indicator, target, target_count, filter);
