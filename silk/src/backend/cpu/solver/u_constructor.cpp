...
/// @brief Construct matrix U by combining r eigenvectors of H (smallest
/// |lambda| of H)
///
...
bool construct_u(const Eigen::SparseMatrix<double>& H, int r,
                 Eigen::MatrixXf& U_out, int max_subspace /* = -1 */,
  ...
  int m = (max_subspace > 0) ? max_subspace
  ...
  const int nconv = static_cast<int>(eigs.compute(
      SortRule::LargestMagn, max_iter, tol, SortRule::LargestMagn));
