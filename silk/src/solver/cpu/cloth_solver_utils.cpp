...
#include "cloth_solver_context.hpp"
...
void batch_reset_cloth_simulation(Registry& registry) {
  for (Entity& e : registry.get_all_entities()) {
    ...
    auto context = registry.get<CpuClothSolverContext>(e);
...
bool prepare_cloth_simulation(Registry& registry, Entity& entity, float dt,
  ...
  if (context && context->dt == dt) {
    ...
    auto new_context = CpuClothSolverContext::make_cloth_solver_context(
        *cloth_config, *topology, *pin, dt);
    if (!new_context) {
      return false;
    }
    context = registry.set<CpuClothSolverContext>(e, std::move(*new_context));
  }
...
void compute_cloth_invariant_rhs(const CpuClothSolverContext& solver_context,
                                 const Pin& pin,
...
bool compute_cloth_outer_loop(
    ...
    CpuClothSolverContext& solver_context, Eigen::Ref<Eigen::VectorXf> rhs) {
...
bool batch_compute_cloth_outer_loop(
  ...
  for (Entity& e : registry.get_all_entities()) {
    ...
    auto dynamic_data = registry.get<CpuClothSolverContext>(e);
...
bool compute_cloth_inner_loop(const ClothConfig& config, const RMatrixX3i& F,
                              ...
                              const CpuClothSolverContext& solver_context,
                              Eigen::Ref<const Eigen::VectorXf> state,
...
bool batch_compute_cloth_inner_loop(Registry& registry,
  ...
  for (Entity& e : registry.get_all_entities()) {
    ...
    auto mesh = registry.get<TriMesh>(e);
    auto topology = registry.get<ClothTopology>(e);
    auto solver_context = registry.get<CpuClothSolverContext>(e);
    auto state = registry.get<ObjectState>(e);
